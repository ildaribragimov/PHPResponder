...
$counMailSend			= 0;			// Общий счетчик отправленных писем
...
function createFile($filename, $fileContent) {
	...
	// Создание файла "stop.txt", чтобы скрипт не срабатывал если работает более раний по времени скрипт
...
function sendMail()	{
	...
	if ( checkFileExists('gotLimit.txt', $gotlimitFileExistlogMessage) && checkFileExists('stop.txt', $stopFileExistlogMessage) && dbConnect() && setParams() && $maxMailSend !== 0 ) {
		...
		if ( $mail = getMailingGroup() ) {
			...
			$needToSend = $maxMailSendInDay + $sendedYesterday - count($whoSent);
			...
			if ( $needToSend > 0 ) {
				...
				if ( !empty($subscribers) ) {
					...
					foreach ( $subscribers as $subscriber ) {
						...
						if ( $counMailSend < $maxMailSend && !in_array($subscriber['email'], $whoSent) ) {
					...
					$query = "UPDATE `".$prefix."subscribers_emails` SET  `log`=".($mail['log']+$counMailSend).", `whoSent`='".implode(',',$whoSent)."', `noteMail`='".$mail['noteMail']."\r\n".date('H:i:s d.m.Y').":Отправлено всего на ".count($whoSent)." e-mail адресов' WHERE `id`=".$mail['id'];
				}
